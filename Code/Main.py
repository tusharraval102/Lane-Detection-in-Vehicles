...
def make_points(image, average, line_length=3/5):
    slope, y_int = average
    y1 = image.shape[0]
...
def video_pipeline(image):
    ...
    # Apply mask
    vertices = np.array(
        [[(650, 800), (950, 600), (1050, 600), (1300, 800)]], dtype=np.int32)
    masked_image = apply_mask(canny, vertices)

    # Apply Hough Line Transformation
    hough = hough_line_transformation(
        masked_image, rho, theta, threshold, min_line_len, max_line_gap)
    lines = cv2.cvtColor(hough, cv2.COLOR_RGB2BGR)
